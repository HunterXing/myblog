	...
	<table>
        ...
        <tbody>
            ...
            <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <td class="name" title="<?php echo ($vol["title"]); ?>"><?php echo (msubstr($vol["title"],0,8)); ?></td>
                    <td class="file"><?php echo ($vol["filename"]); ?> <?php if($vol["hasfile"] == 1): ?><a href="/index.php/Admin/Doc/download/id/<?php echo ($vol["id"]); ?>">【下载】</a><?php else: ?>无附件<?php endif; ?></td>
                    <td class="author"><?php echo ($vol["author"]); ?></td>
                    <td class="addtime"><?php echo (date('Y-m-d H:i:s',$vol["addtime"])); ?></td>
                    <td class="operate">
                        <a href ='javascript:;' class="show" data="<?php echo ($vol["id"]); ?>" data-title="<?php echo ($vol["title"]); ?>" data-content="<?php echo ($vol["content"]); ?>">查看</a>|
